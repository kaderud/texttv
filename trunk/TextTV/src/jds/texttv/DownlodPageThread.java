...
public class DownlodPageThread extends Thread {
	...
	public void run() {
		...
		{
			if (PageNumber[PageListIndex] < 100) {			
			activity.InsertNewPage("Invalid", 0);			
			continue;
			};
			
			URL url;		
	        InputStream urlStream = null;
	        String FeedUrl = "http://svt.se/svttext/tv/pages/" + PageNumber[PageListIndex] + ".html";
	        ...
	        try {
	    		...
	    		{
	    			Log.d(TextTV.TAG,"Texttv data for page " + PageNumber[PageListIndex]+ " found");    			
	    			pagestr = pagestr.substring(PreStart, PreEnd+6);
	    		}
	    		else
	    		{
	    			Log.d(TextTV.TAG,"Could not find data for page " + PageNumber[PageListIndex]+ ". Start = " + PreStart + " End = " + PreEnd);
	    			allreadyRunning = false;
	    			activity.InsertNewPage("Sidan kunde inte laddas",0);
	    			return;
	    		}
	    		...
	    		activity.InsertNewPage(pagestr, PageNumber[PageListIndex]);
